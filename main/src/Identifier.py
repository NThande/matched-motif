...
sliding_lengths = np.arange(0.5, 1.3, 0.1)
count = 0
plt.figure()
...
for i in sliding_lengths:
    ...
    _, windows, matches, _ = autowindow_search(song_fp=audio_fp,
                                               window_length=i,
                                               write_name=file_name,
                                               to_plot=False,
                                               get_info=True)
    matches = matches / i
    plt.plot(windows, matches, 'C{}-'.format(count), label=str(i))
    count += 1
plt.legend()
plt.show()
